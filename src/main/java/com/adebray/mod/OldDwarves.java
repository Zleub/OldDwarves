	...
	public class MyBlock extends Block {
		...
		public class MyBlockEventHandler {
			...
			public void PlayerInteract(PlayerInteractEvent event) {
				instance.events.slots.add(event.action + ":" + event.world.getBlock(event.x, event.y, event.z).getUnlocalizedName());
				if (instance.events.slots.size() >= 24)
					instance.events.slots.remove(0);
				
				nbBlock += 1;
				if (event.action == PlayerInteractEvent.Action.RIGHT_CLICK_BLOCK && event.world.getBlock(event.x, event.y, event.z).getUnlocalizedName().equals(instance.blockA.getUnlocalizedName()))
